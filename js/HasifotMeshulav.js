    ...
    slc.disabled = false;
      ...
      if(product==='קרנות השתלמות' || product==='תגמולים ואישית לפיצויים'
         || product==='קופת גמל להשקעה' || product==='קופת גמל להשקעה - חסכון לילד' ){
          var data=await fetchdataJasonh('../json/dataJasonM.json');
         }
      else if(product==='קרנות חדשות'){
        var data=await fetchdataJasonh('../json/dataJasonP.json');
      }
      else if(product==='פוליסות חסכון'){
        var data=await fetchdataJasonh('../json/dataJasonB.json');
      }
